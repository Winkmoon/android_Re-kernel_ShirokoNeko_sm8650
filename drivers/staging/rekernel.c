...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6,1,0) && LINUX_VERSION_CODE < KERNEL_VERSION(6,13,0)
...
#ifdef CONFIG_IS_LUNARKERNEL
extern bool lunar_verify_key(const char *input);
#endif

static int __init start_rekernel(void)
{
#ifdef CONFIG_IS_LUNARKERNEL
    if (!lunar_verify_key("LunarKernel-Auth-Token:QY#7tW9z!2dF@KL$")) {
		pr_err("This module can only runs on LunarKernel!\n");
        return -ENOEXEC;
	}
#endif
